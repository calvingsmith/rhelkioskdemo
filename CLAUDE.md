...
2. **Isolated per-role sessions via GNOME/GDM headless remote login (recommended direction, pending explicit sign-off)** — confirmed present on both machines: `gnome-remote-desktop.service --system` (system-level RDP daemon, PAM-authenticates against real Linux accounts) + `gnome-headless-session@.service` (templated by account name) + `/usr/libexec/gdm-headless-login-session` (spins up that account's configured session, e.g. `gnome-kiosk-script-wayland`, fully isolated — own compositor, own seat, own D-Bus session bus). Give `user`/`supervisor` each their own Linux account and this sidesteps the seat-merging problem entirely — no shared seat, so no attribution problem in the first place; locking becomes a same-process check in each session's own app instance. Trade: no longer literally one shared framebuffer — needs a small sync layer (shared data source + window-layout messaging between the two app instances, likely a local socket since headless sessions don't share a D-Bus session bus) so both sessions look identical. Not yet verified: whether `gnome-kiosk-script-wayland` runs cleanly when launched this way rather than via physical seat0 autologin.

Status as of 2026-07-09: investigation complete, decision pending — user is taking the memo to the organization before committing to a direction. Don't start building either path without that direction.